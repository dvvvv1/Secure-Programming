#!/bin/bash
# This script is used to attack gedit gdk3 library.
...
mem=`sudo cat /proc/$pid/maps | grep "/" | grep -v "(deleted)" | grep -E "(so|locale)" | grep -E "(r-x)" | grep libgdk-3.so.0.1000.8`
while read -r line; do
  #echo $line
  #echo ./profiling $2 $line
  entry=`./profiling 0 $line 2>&1`
  echo "$entry ./profiling $2 $line"
  ./profiling $2 $line >> $2_gdk3_log.csv
done <<< "$mem"
